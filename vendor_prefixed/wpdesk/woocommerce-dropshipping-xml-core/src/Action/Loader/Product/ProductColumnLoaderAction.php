 ...
 */
class ProductColumnLoaderAction implements Hookable
{
     ...
     */
    private $import_dao;
    public function __construct(Config $config, Request $request, PluginHelper $helper, ImportDAO $import_dao)
    {
        $this->config = $config;
    ...
    public function get_dropshipping_columns($column, $post_id)
    {
        if ($column == self::COLUMN_KEY) {
            ...
            $uid = $product->get_meta(ProductDAO::PRODUCT_IMPORT_ID_META, \true);
