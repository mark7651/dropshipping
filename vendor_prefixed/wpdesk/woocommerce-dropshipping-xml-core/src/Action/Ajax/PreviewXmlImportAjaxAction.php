 ...
 */
class PreviewXmlImportAjaxAction implements Conditional, Hookable
{
    ...
    private function validate_form_data()
    {
        ...
        $item_number = $this->request->get_param('post.' . ImportXmlSelectorFormFields::ITEM_NUMBER);
        if (!$item_number->isSet()) {
            throw new RuntimeException(__('Error: item number is empty.', 'dropshipping-xml-for-woocommerce'));
        }
        $node_element = $this->request->get_param('post.' . ImportXmlSelectorFormFields::NODE_ELEMENT);
    ...
    {
        return ['item_number' => (int) $this->request->get_param('post.' . ImportXmlSelectorFormFields::ITEM_NUMBER)->get(), 'element' => $this->request->get_param('post.' . ImportXmlSelectorFormFields::NODE_ELEMENT)->getAsString(), 'uid' => $this->request->get_param('post.uid')->getAsString()];
    }
}
