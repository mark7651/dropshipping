 ...
 */
class ProductImageMapperService implements ProductMapperServiceInterface
{
    ...
    public function __construct(ImportMapperService $mapper, ProductAttachmentDAO $attachment_dao, ImportLoggerService $logger)
    {
        $this->logger = $logger;
        $this->mapper = $mapper;
    ...
    {
        if ($this->mapper->is_product_field_group_should_be_mapped($wc_product, ImportOptionsFormFields::SYNC_FIELD_OPTION_IMAGES)) {
    ...
    {
        $separator = $this->mapper->get_raw_value(ImportMapperFormFields::PRODUCT_IMAGES_SEPARATOR);
        return $separator === null ? ',' : $separator;
    }
    protected function get_img_field(): string
    {
        return $this->mapper->map(ImportMapperFormFields::PRODUCT_IMAGES);
    }
    protected function is_img_tag_should_be_scanned(): bool
    {
        return CheckboxField::VALUE_TRUE === $this->mapper->get_raw_value(ImportMapperFormFields::PRODUCT_IMAGES_SCAN);
    }
    protected function is_featured_should_by_removed_from_gallery(): bool
    {
        return CheckboxField::VALUE_TRUE === $this->mapper->get_raw_value(ImportMapperFormFields::PRODUCT_IMAGES_FEATURED_NOT_IN_GALLERY);
    }
    protected function is_img_should_be_append(): bool
    {
        return CheckboxField::VALUE_TRUE === $this->mapper->get_raw_value(ImportMapperFormFields::PRODUCT_IMAGES_APPEND_TO_EXISTING);
    }
}
