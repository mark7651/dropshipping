 ...
 */
class XmlSidebarViewerService implements Displayable
{
    const FIRST_PAGE = 1;
    /**
     * @var Renderer
     ...
     */
    private $uid = '';
    public function __construct(Renderer $renderer, DataProviderFactory $data_provider_factory, XmlAnalyser $xml_analyser, FileLocatorService $file_locator, SidebarViewerService $sidebar_viewer_service, string $uid)
    {
    ...
    private function process_xml_file(string $uid): array
    {
        ...
        return ['items' => $this->xml_analyser->count_element($element), 'item_nr' => self::FIRST_PAGE, 'element' => $element, 'content' => $this->xml_analyser->get_element_as_xml($element, self::FIRST_PAGE, \true), 'format' => DataFormat::CSV];
    }
}
