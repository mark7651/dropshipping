...
class PriceModificatorComponent extends \DropshippingXmlFreeVendor\WPDesk\Forms\Field\BasicField
{
    ...
    public function get_price_item_modificator_fields(): array
    {
        if (!isset($this->price_fields)) {
            $this->price_fields = [(new SelectField())->set_name(self::FIELD_PRICE_MOD)->add_class('select short width-100')->set_attribute('type', 'select')->set_options($this->get_price_options()), (new InputNumberField())->add_class('width-100')->set_attribute('step', '0.01')->set_name(self::FIELD_PRICE_MOD_VALUE), (new SelectField())->set_name(self::FIELD_PRICE_TYPE)->add_class('select short width-100')->set_attribute('type', 'select')->set_options($this->get_price_types())];
        }
